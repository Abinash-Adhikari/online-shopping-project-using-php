<?php
    ...
    if(isset($_POST['product-submit'])){
        ...
        $brandName = test_input($_POST['brand-name']);
        $modelNumber = test_input($_POST['model-number']);
        ...
        $paymentMethods = test_input($_POST['payment-methods']);
        $delivaryTime = test_input($_POST['delivary-time']);
        ...
        // && in_array($lpPhotoActualExt,$lpAllowed) && in_array($rpPhotoActualExt,$rpAllowed) 
        //  && ($lpError === 0) && ($rpError === 0) 
        ...
        if(in_array($mpPhotoActualExt,$mpAllowed) && in_array($bpPhotoActualExt,$bpAllowed) && in_array($op1PhotoActualExt,$op1Allowed) && in_array($op2PhotoActualExt,$op2Allowed)){
            if(($mpError === 0)&& ($bpError === 0) && ($bpError === 0) && ($op1Error === 0) && ($op2Error === 0)){
                if(($mpSize < 80000000) && ($bpSize < 80000000) && ($op1Size < 80000000) && ($op2Size < 80000000))
                {
                   ...
                   echo $sql = "INSERT INTO $categoryName (admin_id,status,producttype,productname,brandname,modelnumber,categoryname,price,discount,color,quantity,warrenty,paymentmethods,delivary,features,mainphoto,backphoto,otherphoto1,otherphoto2) 
                    VALUES('$adminId','1','$producttype','$productName','$brandName','$modelNumber','$categoryName','$price','$discount','$color','$quantity','$warrenty','$paymentMethods','$delivaryTime','$features','$mpPhotoDestionation','$bpPhotoDestionation','$op1PhotoDestionation','$op2PhotoDestionation');";
                    $result = mysqli_query($conn,$sql);
                    if($result==TRUE)
                    {
                        header("location: manageProducts.php?table=laptop");
                    }
                    else{
