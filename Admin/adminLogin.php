<?php 
    ...
    if(isset($_POST['loginSubmit'])){
        $email = test_input($_POST['loginname']);
        $password = test_input($_POST['loginPassword']);
        $sql = "SELECT * FROM `controller` WHERE email = '$email' AND password = '$password';";
        ...
        if(in_array($ppPhotoActualExt,$ppAllowed) && in_array($photoActualExt,$allowed)){
            if(($ppError === 0) && ($idError === 0)){
                if(($ppSize < 80000000) && ($idSize < 80000000) ){
                    ...
                    $ppPhotoDestionation = 'documents/'.$ppPhotoNewName;
                    move_uploaded_file($ppTempName,$ppPhotoDestionation);

                    $photoNewName = uniqid('',true).".".$photoActualExt;
                    $photoDestionation = 'documents/'.$photoNewName;
                    move_uploaded_file($idTempName,$photoDestionation);


                    $checksql = "SELECT * FROM `controller` where email = `$email`;";
                    $execute = mysqli_query($conn,$checksql);
                    $c_row = mysqli_fetch_assoc($execute);
                    if(mysqli_num_rows($execute) == 0){
                        $sql = "INSERT INTO `controller` (name,email,password,contact,address,photo,idprove) 
                        ...
                        if($result==TRUE){
                           $x = mt_rand(1000,9999);
                            ...
                            if(mail($to_email, $subject, $body, $headers)) {
                                ...
                                $_SESSION['user_id'] = $c_row['user_id'];
                                echo header("location: adminEmailActive.php");
                            } else {
                                echo 
                                '
                                <script>
...
<body>
    <div class="login-signup">
        <div class="img">
            <a href="../Frontend/index.php"><img src="./Photos/logo2.png" alt=""></a>
        </div>
        <div class="rotate">
